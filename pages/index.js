...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    return Object.fromEntries(Object.entries(MENU_DATA).map(([category, items]) => [category, items.filter(item => `${item.name} ${item.desc || ''}`.toLowerCase().includes(term))]).filter(([, items]) => items.length));
  }, [search]);
  return (<main className="page"><div className="glow glowOne"/><div className="glow glowTwo"/><header className="hero"><div className="logoMark" aria-label="Mordisko"><span>M</span></div><div className="brand">MORDISKO</div><div className="tagline">SABOR QUE SE MORDISKEA</div><div className="goldLine"/><h1>NUESTRO MENÚ</h1><p>Elige tu favorito y disfruta el sabor Mordisko.</p></header><div className="searchWrap"><span className="searchIcon">⌕</span><input value={search} onChange={e => setSearch(e.target.value)} placeholder="¿Qué se te antoja hoy?" aria-label="Buscar en el menú"/>{search && <button onClick={() => setSearch('')} aria-label="Limpiar búsqueda">×</button>}</div><div className="menu">{Object.entries(filtered).map(([category, items]) => <section className="category" key={category}><div className="categoryTitle"><span>{category}</span><i/></div><div className="itemsGrid">{items.map((item, index) => <article className="card" key={`${item.name}-${index}`}><div className="cardTop"><h2>{item.name}</h2><strong>{item.price}</strong></div>{item.desc && <p>{item.desc}</p>}</article>)}</div></section>)}{!Object.keys(filtered).length && <div className="empty">No encontramos ese antojo 😕</div>}</div><footer>MORDISKO · HECHO PARA MORDERSE LOS DEDOS</footer><style jsx global>{`*{box-sizing:border-box}html,body,#__next{margin:0;min-height:100%}body{background:#090909;color:#fff;font-family:Arial,Helvetica,sans-serif}button,input{font:inherit}.page{min-height:100vh;width:100%;position:relative;overflow:hidden;padding:34px clamp(18px,4vw,70px) 28px;background:radial-gradient(circle at top,#252000 0,#111 30%,#090909 72%)}.glow{position:absolute;width:420px;height:420px;border-radius:50%;filter:blur(90px);opacity:.12;pointer-events:none;background:#FFD700}.glowOne{top:-250px;left:-180px}.glowTwo{bottom:-280px;right:-170px}.hero{position:relative;text-align:center;max-width:1200px;margin:0 auto 24px}.logoMark{width:76px;height:76px;margin:0 auto 9px;border:3px solid #FFD700;border-radius:50%;display:grid;place-items:center;box-shadow:0 0 30px rgba(255,215,0,.2),inset 0 0 18px rgba(255,215,0,.08)}.logoMark span{color:#FFD700;font-weight:900;font-size:46px;line-height:1;font-style:italic}.brand{color:#FFD700;font-size:clamp(36px,6vw,68px);line-height:.95;font-weight:950;letter-spacing:.08em;font-style:italic;text-shadow:0 4px 22px rgba(255,215,0,.16)}.tagline{margin-top:9px;color:#d7d7d7;font-size:11px;letter-spacing:.28em;font-weight:700}.goldLine{width:90px;height:3px;background:#FFD700;margin:18px auto 15px;border-radius:99px}.hero h1{margin:0;font-size:clamp(24px,3vw,36px);letter-spacing:.04em}.hero p{color:#aaa;margin:7px 0 0;font-size:14px}.searchWrap{position:relative;max-width:920px;margin:0 auto 38px;display:flex;align-items:center;background:#171717;border:1px solid #333;border-radius:16px;min-height:58px;box-shadow:0 12px 35px rgba(0,0,0,.35)}.searchWrap:focus-within{border-color:#FFD700;box-shadow:0 0 0 3px rgba(255,215,0,.08),0 12px 35px rgba(0,0,0,.35)}.searchIcon{color:#FFD700;font-size:28px;margin-left:18px;transform:rotate(-20deg)}.searchWrap input{flex:1;min-width:0;border:0;outline:0;background:transparent;color:#fff;padding:0 14px;font-size:16px}.searchWrap input::placeholder{color:#777}.searchWrap button{border:0;background:transparent;color:#888;font-size:27px;padding:0 18px;cursor:pointer}.menu{position:relative;max-width:1500px;margin:0 auto}.category{margin-bottom:42px}.categoryTitle{display:flex;align-items:center;gap:16px;margin-bottom:17px;color:#FFD700;font-size:clamp(20px,2vw,27px);font-weight:900;letter-spacing:.02em}.categoryTitle i{height:1px;background:linear-gradient(90deg,#FFD700,transparent);flex:1;opacity:.5}.itemsGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.card{background:linear-gradient(145deg,rgba(255,255,255,.055),rgba(255,255,255,.018));border:1px solid rgba(255,215,0,.16);border-radius:18px;padding:18px;min-height:118px;box-shadow:0 12px 28px rgba(0,0,0,.25)}.cardTop{display:flex;justify-content:space-between;gap:14px;align-items:flex-start}.card h2{font-size:16px;line-height:1.15;margin:0;color:#fff}.card strong{color:#FFD700;font-size:16px;white-space:nowrap}.card p{margin:10px 0 0;color:#999;font-size:13px;line-height:1.45}.empty{text-align:center;color:#aaa;padding:50px 0}.page footer{position:relative;text-align:center;color:#666;font-size:10px;letter-spacing:.18em;margin-top:18px}@media(max-width:900px){.itemsGrid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:600px){.page{padding:24px 14px}.itemsGrid{grid-template-columns:1fr}.card{min-height:0}.searchWrap{margin-bottom:30px}.category{margin-bottom:34px}}`}</style></main>);
}
